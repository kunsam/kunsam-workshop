...
function PointMass(cx, cy, mass) {
  ...
  this.move = function (dt) {
    ...
    t =
      (2.0 - this.friction) * c -
      (1.0 - this.friction) * this.prev.getX() +
      a * dtdt;
    this.prev.setX(c);
    this.cur.setX(t);
    ...
    t =
      (2.0 - this.friction) * c -
      (1.0 - this.friction) * this.prev.getY() +
      a * dtdt;

    // console.log(c, t, dt, t - c, "timeout");
    this.prev.setY(c);
    this.cur.setY(t);
  };
...
function Stick(pointMassA, pointMassB) {
  ...
  this.draw = function (ctx, scaleFactor) {
    ...
    ctx.beginPath();
    ctx.moveTo(
      this.pointMassA.getXPos() * scaleFactor,
...
function Blob(x, y, radius, numPointMasses) {
  ...
  for (i = 0; i < numPointMasses; i++) {
    this.sticks[i] = new Stick(
      this.pointMasses[i],
      this.pointMasses[clampIndex(i + 1, numPointMasses)]
    );
  }
  ...
  for (i = 0, p = 0; i < numPointMasses; i++) {
    this.joints[p++] = new Joint(
      this.pointMasses[i],
      this.pointMasses[clampIndex(i + numPointMasses / 2 + 1, numPointMasses)],
      low,
      high
    );
    this.joints[p++] = new Joint(
  ...
  this.sc = function (env) {
    ...
    for (i = 0; i < this.pointMasses.length; i++) {
      if (
        env.collision(
          this.pointMasses[i].getPos(),
          this.pointMasses[i].getPrevPos()
        ) == true
      ) {
    ...
    // let string = "";
    // this.pointMasses.forEach((pt) => {
    //   string += `circle(${pt.cur.x * scaleFactor}|${
    //     pt.cur.y * scaleFactor
    //   } radius)`;
    // });
  };
  this.setForce = function (force) {
  ...
  this.drawBody = function (ctx, scaleFactor) {
    ...
    ctx.beginPath();
    ctx.moveTo(
      this.pointMasses[0].getXPos() * scaleFactor,
  ...
  this.draw = function (ctx, scaleFactor) {
    ...
    // ctx.save();
    // ctx.translate(
    //   this.middlePointMass.getXPos() * scaleFactor,
...
function BlobCollective(x, y, startNum, maxNum) {
  ...
  this.blobs[0] = new Blob(x, y, 30, 8);
...
function update() {
  ...
  if (savedMouseCoords != null && selectOffset != null) {
    blobColl.selectedBlobMoveTo(
      savedMouseCoords.x - selectOffset.x,
...
function init() {
  ...
  function getMouseCoords(event) {
    ...
    if (touch.pageX || touch.pageY) {
      return { x: touch.pageX / scaleFactor, y: touch.pageY / scaleFactor };
    }
    return null;
  }
  ...
  env = new Environment(0, 0, 3.75 * 100, 6.67 * 100);
  blobColl = new BlobCollective(2.0 * 100, 3.0 * 100, 1, 100);
